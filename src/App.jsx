...
/* ═══════════════════════════════════════════
   ...
   ═══════════════════════════════════════════ */
const FONT =
  "https://fonts.googleapis.com/css2?family=Playfair+Display:ital,wght@0,400;0,700;0,900;1,400&family=JetBrains+Mono:wght@400;500;600;700&display=swap";
...
const C = {
  ...
  bdr: "rgba(212,168,73,0.2)",
  bdrA: "rgba(212,168,73,0.55)",
  win: "#f1c40f",
  aceL: "#5dade2",
  aceH: "#e74c3c",
};
      ...
      <div
        ...
        style={{
          ...
          background: src
            ...
            : `linear-gradient(135deg, ${C.gold}44, ${C.goldDim}66)`,
...
const btnGold = {
  ...
  color: C.dark,
  cursor: "pointer",
  boxShadow: "0 4px 24px rgba(212,168,73,0.3)",
  transition: "all 0.2s",
};
...
export default function App() {
  ...
  // Card value settings
  const [scoringMode, setScoringMode] = useState("standard"); // "standard" | "faceValue" | "custom"
  ...
  useEffect(() => {
    setSetupPlayers((prev) => {
      ...
      for (let i = 0; i < playerCount; i++)
        next.push(prev[i] || { name: "", photo: null, save: false });
      return next;
    });
  }, [playerCount]);
  ...
  /* ── Computed ── */
  const getTotal = (pid) =>
    rounds.reduce((s, r) => s + (r.scores[pid] || 0), 0);
  ...
  const resolvedPointGoal = [100, 200, 300].includes(pointGoal)
    ? pointGoal
    : parseInt(customPointGoal) || 100;
  const resolvedRoundGoal = [5, 10, 15].includes(roundGoal)
    ? roundGoal
    : parseInt(customRoundGoal) || 10;
    ...
    if (toSave.length > 0) {
      ...
      toSave.forEach((sp) => {
        const existing = updated.findIndex(
          (s) => s.name.toLowerCase() === sp.name.trim().toLowerCase(),
        );
        if (existing >= 0) {
          updated[existing] = {
            name: sp.name.trim(),
            photo: sp.photo || updated[existing].photo,
          };
        } else {
  ...
  const submitRound = () => {
    const scores = {};
    players.forEach((p) => {
      scores[p.id] = parseInt(roundInputs[p.id]) || 0;
    });
      ...
      <div style={shell}>
        ...
        <div style={wrap}>
          ...
          >
            ...
            {/* New Session */}
            <button
              style={{ ...btnGold, animation: "fadeUp 0.6s ease 0.15s both" }}
            ...
            <div
              style={{ width: "100%", animation: "fadeUp 0.6s ease 0.3s both" }}
            >
              <button
                ...
                style={{
                  ...
                  color: showSaved ? C.gold : C.creamD,
                  cursor: "pointer",
                  transition: "all 0.2s",
                  display: "flex",
                  ...
                  {savedPlayers.length === 0 ? (
                    ...
                    >
                      ...
                      <span style={{ color: C.goldDim }}>
                        Toggle "Save" on a player during session setup
                      </span>
                    </div>
                  ) : (
                    <div
                        ...
                        >
                          ...
                          {editingSaved === i ? (
                            <input
                              ...
                              autoFocus
                              onChange={(e) =>
                                updateSavedName(i, e.target.value)
                              }
                              onBlur={() => setEditingSaved(null)}
                              onKeyDown={(e) =>
                                e.key === "Enter" && setEditingSaved(null)
                              }
                            />
                          ) : (
                            <span
        ...
        <div style={wrap}>
          ...
          >
            ...
            >
                  ...
                  {setupPlayers.map((sp, i) => (
                    ...
                    >
                      <Avatar
                        ...
                        editable
                        onClick={(photo) =>
                          updateSetupPlayer(i, "photo", photo)
                        }
                      />
                      <div
                        style={{
                      ...
                      >
                        <input
                          ...
                          value={sp.name}
                          onChange={(e) =>
                            updateSetupPlayer(i, "name", e.target.value)
                          }
                          onFocus={(e) => (e.target.style.borderColor = C.gold)}
                          onBlur={(e) => (e.target.style.borderColor = C.bdr)}
                        />
                        ...
                        >
                          <button
                            onClick={() =>
                              updateSetupPlayer(i, "save", !sp.save)
                            }
                            style={{
                              padding: "4px 10px",
                              fontSize: "9px",
                          ...
                          {savedPlayers.length > 0 && (
                            <select
                              ...
                              onChange={(e) => {
                                ...
                                if (!isNaN(idx))
                                  addSavedToSetup(savedPlayers[idx], i);
                              }}
                            >
                              <option value="">Load saved...</option>
                              {savedPlayers.map((s, si) => (
              ...
              {/* Scoring Mode */}
              <div>
                <span style={label}>Card Scoring</span>
                <div
                  style={{ display: "flex", gap: "8px", marginBottom: "12px" }}
                >
                  <button
                    style={pill(scoringMode === "standard")}
                    onClick={() => {
                  ...
                  <button
                    style={pill(scoringMode === "faceValue")}
                    onClick={() => {
                      setScoringMode("faceValue");
                      setLowCardValue(0);
                      setFaceCardValue(10);
                      setAceValue(1);
                    }}
                ...
                {scoringMode === "standard" && (
                  ...
                  >
                    Classic Gin Rummy: 2-9 = 5 pts, 10/J/Q/K = 10 pts. Choose
                    Ace value below.
                  </div>
                )}

                {scoringMode === "faceValue" && (
                  <div
                    style={{
                      fontSize: "11px",
                      color: C.creamD,
                  ...
                  >
                    Each card equals its number: 2 = 2, 3 = 3, etc. Face cards =
                    10. Ace = 1.
                  </div>
                )}
                ...
                {scoringMode === "custom" && (
                  ...
                  >
                    Set your own point values below.
                  </div>
                )}
                ...
                <div style={{ marginBottom: "12px" }}>
                  ...
                  <div style={{ display: "flex", gap: "8px" }}>
                    {[1, 5, 10, 15].map((n) => (
                      <button
                        key={n}
                        style={pill(aceValue === n)}
                        onClick={() => setAceValue(n)}
                      >
                        {n}
                      </button>
                    ))}
                    ...
                    <div style={{ marginBottom: "12px" }}>
                      ...
                      <div style={{ display: "flex", gap: "8px" }}>
                        {[1, 5, 10].map((n) => (
                          <button
                            key={n}
                            style={pill(lowCardValue === n)}
                            onClick={() => setLowCardValue(n)}
                          >
                            {n}
                          </button>
                        ))}
              ...
              <div>
                ...
                {goalType === "points" && (
                  <>
                    ...
                    >
                      First player to reach the target score wins the session.
                      The standard in Gin Rummy is 100 points — but set it to
                    ...
                    >
                      ...
                      <input
                        style={{
                          ...
                          textAlign: "center",
                          borderColor:
                            ![100, 200, 300].includes(pointGoal) &&
                            customPointGoal
                              ? C.gold
                              : C.bdr,
                        }}
                        placeholder="Custom"
                        value={customPointGoal}
                        type="number"
                        inputMode="numeric"
                        onChange={(e) => {
                          setCustomPointGoal(e.target.value);
                          setPointGoal(-1);
                        }}
                      />
                    </div>
                ...
                {goalType === "rounds" && (
                  <>
                    ...
                    >
                      Play a fixed number of hands. After the final round, the
                      player with the highest total deadwood score takes the
                      crown.
                    </div>
                    <div
                      style={{ display: "flex", gap: "8px", flexWrap: "wrap" }}
                    >
                      ...
                      <input
                        style={{
                          ...
                          textAlign: "center",
                          borderColor:
                            ![5, 10, 15].includes(roundGoal) && customRoundGoal
                              ? C.gold
                              : C.bdr,
                        }}
                        placeholder="Custom"
                        value={customRoundGoal}
                        type="number"
                        inputMode="numeric"
                        onChange={(e) => {
                          setCustomRoundGoal(e.target.value);
                          setRoundGoal(-1);
                        }}
                      />
                    </div>
                ...
                {goalType === "free" && (
                  ...
                  >
                    No target — play as many hands as you like. End the session
                    whenever you're done. The player with the most points gets
                    the crown.
                  </div>
                )}
              </div>
            ...
            >
              <span style={{ ...label, marginBottom: "10px" }}>
                Deadwood Card Values
              </span>
              <div
                style={{
                  display: "flex",
            ...
            <button style={btnGold} onClick={startSession}>
              Start Session →
            </button>
          </div>
        </div>
        ...
        <div style={wrap}>
          ...
          >
                ...
                return (
                  <div
                    ...
                    style={{
                      ...
                      gap: "14px",
                      padding: "16px 18px",
                      background: isChamp ? "rgba(241,196,15,0.08)" : C.card,
            ...
            <div style={{ fontSize: "11px", color: C.creamD }}>
              {rounds.length} hand{rounds.length !== 1 ? "s" : ""} played · Ace
    ...
    <div style={shell}>
      ...
      <div style={wrap}>
        ...
        >
          ...
          <button
            style={{ ...btnOutline(C.redS), padding: "8px 14px" }}
            onClick={endSession}
          >
            End
          </button>
        ...
        >
          {players.map((p, i) => {
            const total = getTotal(p.id);
            const pct =
              gameGoalType === "points" && gameTarget > 0
                ? (total / gameTarget) * 100
                : 0;
            const isCrown = leader.id === p.id && !leader.tied;
            const lastR =
              rounds.length > 0 ? rounds[rounds.length - 1].scores[p.id] : null;
        ...
        {showRoundEntry && (
          ...
          >
            ...
            >
              <span style={{ ...label, margin: 0, fontSize: "11px" }}>
                Hand {rounds.length + 1} — Deadwood Scores
              </span>
              <button
                style={btnOutline()}
                onClick={() => setShowRoundEntry(false)}
              >
                Cancel
              </button>
            </div>
            ...
            >
              <span style={{ color: C.cream }}>2-9</span> = 5 ·{" "}
              <span style={{ color: C.cream }}>10,J,Q,K</span> = 10 ·{" "}
              <span style={{ color: gameAceRule === "high" ? C.aceH : C.aceL }}>
                A = {gameAceRule === "high" ? "15" : "5"}
              </span>
            </div>
            <div
        ...
        >
          <button
            style={{
              ...btnOutline(),
              opacity: rounds.length === 0 ? 0.25 : 1,
              cursor: rounds.length === 0 ? "default" : "pointer",
            }}
            onClick={undoLast}
          ...
          <button style={btnOutline(C.redS)} onClick={endSession}>
            End Session
          </button>
        </div>
        ...
        {rounds.length > 0 && (
          <button
            style={{
              ...
              color: showHistory ? C.gold : C.creamD,
              cursor: "pointer",
              marginTop: "6px",
              transition: "all 0.2s",
            }}
              ...
              >
                ...
                <tbody>
                  {rounds.map((r, ri) => (
                    <tr
                      key={ri}
                      style={{ borderBottom: `1px solid ${C.bdr}11` }}
                    >
                      <td
                        style={{
                          padding: "9px 12px",
                      ...
                      {players.map((p) => (
                        <td
                          ...
                          style={{
                            padding: "9px 8px",
                            textAlign: "center",
                            color: r.scores[p.id] > 0 ? C.cream : C.creamD,
                            fontWeight: r.scores[p.id] > 0 ? 600 : 400,
                          }}
                        >
                          {r.scores[p.id] > 0
                            ? `+${r.scores[p.id]}`
            ...
            {sessionWinner.photo && (
              <div
                style={{
                  ...
                  borderRadius: "50%",
                  margin: "0 auto 12px",
                  background: `url(${sessionWinner.photo}) center/cover`,
                  border: `3px solid ${C.win}`,
